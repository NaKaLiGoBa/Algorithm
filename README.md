...
### 알고리즘 유형 분류
> 아래와 같은 유형들로 분류를 하였고, 순서를 정했다. 목록의 순서는 빈출도와 중요도를 고려하여 작성되었다.
1. 수학 및 문자열
2. 자료구조 - 스택, 큐, 맵
3. DFS, BFS, 그래프 이론
4. 완전탐색, 백트래킹
...
* 자료구조의 구현 방식과 메서드 이름의 차이가 있을 수 있고, 다니나믹 혹은 스태틱 타이핑에서 오는 차이 및 OOP, 함수형에서 오는 차이가 있다.
* 하지만, 문제를 푸는 논리는 공통적이기 때문에 큰 문제가 없이 이해할 수 있었다.
* 문제점은 각 언어의 best practice가 있는데, 언어가 다르면 피드백을 해주기 어려울 수 있다는 점이다.
<br>
